...
class MockMarketDataClient(MarketDataClient):
    ...
    def request_instrument(self, request: RequestInstrument) -> None:
        self._handle_instrument(self.instrument, request.id, request.params, request.start, request.end)

    def request_instruments(self, request: RequestInstruments) -> None:
        self._handle_instruments(request.venue, self.instruments, request.id, request.params, request.start, request.end)
